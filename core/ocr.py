...
class OCRProcessor:
    ...
    def __init__(self, lang='en', reading_direction='auto'):
        ...
        print(f"[OCR] Inicializando idioma={paddle_lang}, direção={self.reading_direction} (auto-detectado)" if reading_direction == 'auto' else f"[OCR] Inicializando idioma={paddle_lang}, direção={self.reading_direction}")
    ...
    def _optimize_image(self, img_array):
        ...
        if max_dim > self.MAX_IMAGE_DIM:
            ...
            print(f"[OCR] Imagem redimensionada: {w}x{h} -> {new_w}x{new_h} (fator: {self.scale_factor:.2f})")
        else:
            self.scale_factor = 1.0
    ...
    def process_image(self, image):
        ...
        print(f"[{time.strftime('%H:%M:%S')}] [OCR] PaddleOCR concluído em {ocr_time:.2f}s")
        
        # Parse results
        parsed_results = self._parse_ocr_results(result)
        
        total_time = time.time() - start_time
        print(f"[{time.strftime('%H:%M:%S')}] [OCR] Processamento concluído: {len(parsed_results)} textos em {total_time:.2f}s")
        
        # Group text into speech bubbles using clustering
        grouped_results = self._group_text_bubbles(parsed_results)
        print(f"[{time.strftime('%H:%M:%S')}] [OCR] Agrupamento: {len(parsed_results)} -> {len(grouped_results)} balões")
    ...
    def _group_text_bubbles(self, results):
        ...
        for i in range(n):
            ...
            while queue:
                ...
                for j in range(n):
                    ...
                    if self._boxes_overlap_or_close(results[current]['bbox'], results[j]['bbox']):
        ...
        for group_indices in groups:
            ...
            # Average confidence
            avg_confidence = sum([box['confidence'] for box in group_boxes]) / len(group_boxes)
